...
function prepare_batch_insert()
   for t = 1, sysbench.opt.batch_inserts do
      stmt_defs["batch_inserts"][1] = stmt_defs["batch_inserts"][1]..string.format("(%d,%s,%s)", t, "0", "'aurora rr test'")
      if t ~= sysbench.opt.batch_inserts then
         stmt_defs["batch_inserts"][1] = stmt_defs["batch_inserts"][1]..","
      end
   end
   prepare_for_each_table("batch_inserts")
end
...
function execute_batch_insert()
   local tnum = get_table_num()
   stmt[tnum].batch_inserts:execute()
end
